...
def Strain_E(node_coords, connectivity, fixed_dof, F):
    ...
    return Strain_energy, displacements, lens, V, FE_timing
...
for iteration in range(epochs + 1):
    ...
    ####### FDM time
    ini_time = (time.time() - iter_start) / 60
        
    N_coords = new_node_coords.clone()
    FE_str = time.time()
    Strain_energy, _, _, _, FE_timing = Strain_E(N_coords, connectivity, fixed_dof, F_fe_g)
